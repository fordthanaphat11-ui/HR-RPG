(function (window, document) {
    ...
    const UI = {
        ...
        bindAdminMapEvents() {
            ...
            document.querySelectorAll('[data-geofence-add]').forEach(function (button) { button.addEventListener('click', function () {
            ...
            document.querySelectorAll('[data-geofence-edit]').forEach(function (button) { button.addEventListener('click', function () { if (self.confirmDiscard()) self.startEdit(button.dataset.geofenceEdit); }); });
        ...
        confirmDiscard() {
            return !this.dirty || window.confirm('มีการแก้ไขพื้นที่ที่ยังไม่ได้บันทึก ต้องการยกเลิกการแก้ไขหรือไม่?');
        },

        cancelEdit(requireConfirm) {
            if (requireConfirm && !this.confirmDiscard()) return;
            this.stopEditing();
        ...
        centerCurrentLocation(button) {
            ...
            }, function () { button.disabled = false; button.innerHTML = original; window.alert('ไม่สามารถอ่านตำแหน่งปัจจุบันได้ กรุณาอนุญาต GPS แล้วลองใหม่'); }, { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 });
        },
